...
type EventType = "KeyDown" | "KeyUp" | "MouseMove" | "MouseDown" | "MouseUp"
interface Event {
    eventType: EventType,
...
class _CGame {
    ...
    getMousePos(event: MouseEvent) {
        ...
        if (x < 0 || x > this.ctx.canvas.width || y < 0 || y > this.ctx.canvas.height) return
    ...
    private alreadyFired: Map<string, boolean>
    ...
    constructor(private _startingScene: typeof BaseScene, public ctx: CanvasRenderingContext2D, public fps: number, public width: number, public height: number) {
        ...
        document.onkeyup = (event) => {
            ...
            this.eventQueue.push(<KeyUpEvent>{ eventType: "KeyUp", key: event.key, code: event.code, raw: event })
        }

        /* ------------------------------ Mouse events ------------------------------ */
        document.onmousemove = (event) => {
            if (!this.getMousePos(event)) return
            this.mouse = this.getMousePos(event)
            this.eventQueue.push(<MouseMoveEvent>{ eventType: "MouseMove", raw: event })
        }
        ...
        document.onmousedown = (event) => {
            ...
            this.eventQueue.push(<MouseDownEvent>{ eventType: "MouseDown", raw: event })
        }
        document.onmouseup = (event) => {
            ...
            this.eventQueue.push(<MouseUpEvent>{ eventType: "MouseUp", raw: event })
        }
    }
